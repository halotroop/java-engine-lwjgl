	...
	protected void moveWindow(InputHandler input, GUIBoundsInterface bounds)
	{
		...
		if (withinBounds((float) input.getMouseX(), (float) input.getMouseY()) || moving)
		{
			...
			if (input.isMouseDown(LWJGLResourceManager.instance().inputMapping().getBinding("mouse_left")))
			{
				...
				if (keepWithinParent)
				{
					if (getWidth() / scaleX + xOffset > bounds.getBounds().x / scaleX)
						xOffset = (bounds.getBounds().x - getWidth()) / scaleX;
					else if (xOffset < 0) xOffset = 0;
					if (getHeight() / scaleY + yOffset > bounds.getBounds().y / scaleY)
						yOffset = (bounds.getBounds().y - getHeight()) / scaleY;
