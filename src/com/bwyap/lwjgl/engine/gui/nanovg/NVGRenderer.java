...
public class NVGRenderer extends GUIRenderer
{
	...
	public void renderPanel(Panel panel, WindowInterface window, Vector2f parent)
	{
		...
		while (it.hasNext())
		{
			...
			if (e instanceof Button)
				renderButton((Button) e);
			else if (e instanceof VectorScrollArea)
				renderVectorScrollArea((VectorScrollArea) e, window);
			else if (e instanceof VectorCheckBox)
				renderVectorCheckBox((VectorCheckBox) e);
			else if (e instanceof VectorRadioButton)
				renderVectorRadioButton((VectorRadioButton) e);
			else if (e instanceof RadioButtonGroup)
				renderRadioButtonGroup((RadioButtonGroup) e, panel);
			else if (e instanceof VectorProgressBar)
				renderVectorProgressBar((VectorProgressBar) e);
			else if (e instanceof Label)
				renderLabel((Label) e);
			else if (e instanceof VectorTextBox)
				renderVectorTextBox((VectorTextBox) e);
			else if (e instanceof VectorTextField)
				renderVectorTextField((VectorTextField) e);
			else if (e instanceof ImageHolder)
				renderImage((ImageHolder) e);
	...
	public void renderButton(Button button)
	{
		...
		if (button instanceof TexturedButton)
			renderTexturedButton((TexturedButton) button);
	...
	public void renderText(TextComponent text, GUIBoundsInterface e)
	{
		...
		if (text.scale())
		{
			...
			// Wider
			if (AR > originalAR)
				scaleFactor = e.getBounds().y / e.getAbsoluteBounds().y;
			// Thinner
		...
		if (text.isTextBox())
			nvgTextBox(getContext(), xPos, yPos, text.getTextBoxWidth(), text.getText());
	...
	protected void renderColouredVectorShape(IColouredVectorShape shape, GUIBoundsInterface e, float offsetX, float offsetY)
	{
		...
		{
			if (mouseDown)
				nvgFillColor(getContext(), rgba(shape.colourProperties().getMouseDownColour()));
		...
		{
			if (mouseOver)
				nvgStrokeColor(getContext(), rgba(shape.colourProperties().getBorderMouseoverColour()));
	...
	protected void renderTexturedButton(TexturedButton button)
	{
		...
		if (button.isMouseOver())
		{
			if (button.isMouseDown())
				nvgImagePattern(getContext(), button.getPositionX(), button.getPositionY(), button.getWidth(), button.getHeight(), 0, NVGTexture.getTexture(button.getPressedTexture()), 1, paint);
